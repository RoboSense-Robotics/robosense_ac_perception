...
void PromptDAConfig::Init(const YAML::Node &cfg_node) {
...
#ifdef __aarch64__
#ifdef ENABLE_RKNN
    rally::yamlRead(cfg_node, "arm_rknn_model_s1",s1_model_path);
    rally::yamlRead(cfg_node, "arm_rknn_model_s2",s2_model_path);
#endif
#ifdef ENABLE_HBDNN
    ...
    rally::yamlRead(cfg_node, "calib_file", calib_file);
#if defined(USE_ROS1)
    calib_file = std::string(PROJECT_PATH) + "/../" + calib_file;
#elif defined(USE_ROS2)
    calib_file = std::string(buffer) + "/" + calib_file;
#endif
    std::ifstream ifs_calib(calib_file);
    if (!ifs_calib.is_open()) {
        std::cout << "calib file path error!" << std::endl;
    } else {
        std::cout << "calib file path is: " << calib_file << std::endl;
        ifs_calib.close();
    }
    rally::yamlRead(cfg_node, "nn_input_width", nn_input_width);
    rally::yamlRead(cfg_node, "nn_input_height", nn_input_height);
    rally::yamlRead(cfg_node, "clip_min", clip_min);
    rally::yamlRead(cfg_node, "clip_max", clip_max);
    rally::yamlRead(cfg_node, "core", core);
